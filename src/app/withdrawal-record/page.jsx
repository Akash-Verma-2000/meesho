...
export default function WithdrawalRecordPage() {
    ...
    const fetchRecords = async () => {
        ...
        try {
            ...
            const response = await fetch(`/api/user/transaction?page=${currentPage}&limit=${recordsPerPage}&type=withdraw`, {
                headers: {
            ...
            if (data.status === 'success') {
                ...
                setError(data.message || 'Failed to fetch withdrawal records');
                toast.error(data.message || 'Failed to fetch withdrawal records', { position: "top-right" });
            ...
            setError('An unexpected error occurred while fetching withdrawal records');
            toast.error('An unexpected error occurred while fetching withdrawal records', { position: "top-right" });
        } finally {
            setLoading(false);
        }
    };
        ...
        return (
            <WebsiteLayout>
                <div className="min-h-screen bg-gray-100 flex items-center justify-center">
                    <p>Loading withdrawal records...</p>
                </div>
            </WebsiteLayout>
        );
    }
